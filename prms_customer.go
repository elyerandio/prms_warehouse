...
type Customer struct {
	...
	MTDC  float64   `db:"MTD\$C"`
	YTDC  float64   `db:"YTD\$C"`
	...
	MDCC  float64   `db:"MDC\$C"`
	YDCC  float64   `db:"YDC\$C"`
	...
	CMRGL int       `db:"CMRGL"`
	CFCM  float64   `db:"CFC\$M"`
	CFCY  float64   `db:"CFC\$Y"`
	...
	FMTC  float64   `db:"FMT\$C"`
	FYTC  float64   `db:"FYT\$C"`
...
func processCustomerTable() {
	...
	insertStmt := fmt.Sprintf("INSERT INTO %s (%s) VALUES(%s)", newCustomerTable, fieldsCSV,
		fieldsCSVColons)
	recCount := 0
	fmt.Printf("\nTable Name : %s\n", newCustomerTable)
	...
	for rows.Next() {
		...
		_, err = dbMysql.NamedExec(insertStmt, cust)
		if err != nil {
			panic(err)
		}
	}
